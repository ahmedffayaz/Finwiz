...
class PlaidApiController extends Controller
{
    ...
    public function plaidAuthGet()
    {
        ...
        try {
            if (isset($plaid_access_token)) {
                ...
                self::plaidInvestmentTransactionGet();
            ...
            foreach ($data['added'] as $key => $value) {
                ...
                if ($category_id) {
                    $transaction = Transaction::where('user_id', $user_id)
                        ...
                        ->first();
                    //For testing purposes it is commented out:
                    ...
                    if ($transaction) {
                        $transaction->update([
                            ...
                            //For testing purposes it is commented out:
                            // 'amount' => $value['amount'],
                            'amount' => ($value['amount'] < 0) ? $value['amount'] : -$value['amount'],
    ...
    public function updateLimit($data)
    {
        $userId = $data->user_id;
        ...
        // if ($auth) {
        //     $auth->balances_available += $amount;
        //     $auth->balances_current += $amount;
        //     $auth->update();
        // }
    }
    /**
